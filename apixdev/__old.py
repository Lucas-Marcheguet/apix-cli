...
class Apix(metaclass=SingletonMeta):
            ...
            vals.update(
                {
                    ...
                    if k not in cfg.IGNORED_VALUES
                }
            )
    ...
    def get_missing_values(self):
        ...
        for key in cfg.MANDATORY_VALUES:
            if key not in vars or not vars.get(key, False):
                missing_values[key] = ""
    ...
    def _get_odoo(self):
        ...
        params = {
            k: v
            for k, v in self._config["apix"].items()
            if k in ["port", "protocol", "timeout"]
        }
    ...
    def get_ticket(self, id):
        ...
        database_type = (
            "Production" if ticket.statut_odoo == "Production" else "Recette"
        )
        domain = [
            ("partner_id", "in", ticket.partner_instance.ids),
            ("database_type_id.name", "=", database_type),
        ]
        ...
        branch_dev = "{}-dev-{}".format(
            database.odoo_version_group_id.major_version, ticket.id
        )
    ...
    def project_cmd(self, name, action, **kwargs):
        ...
        if action == "run":
            ...
            raise NotImplementedError("Command {} not implemented.".format(action))

        # docker ps --format "{{json .}}"

        _logger.info(
            "Action {} on project {}.\n\tArguments: {}\n\tOptions: {}.".format(
                action, name, dict_to_string(kwargs), dict_to_string(params)
            )
        )
