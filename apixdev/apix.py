...
from apixdev.common import Config, Odoo, SingletonMeta
...
class Apix(metaclass=SingletonMeta):
    ...
    def is_ready(self):
        return (
            True
            if self.config and len(self.config.get_missing_values()) == 0
            else False
        )
    ...
    def prepare_compose_vals(self, database, requirements, major_version=True):
        requirements += ["inotify", "pdfminer.six"]

        if major_version:
            docker_image = (
                "{o.docker_image}:{o.major_version}-{o.docker_tag_suffix}".format(
                    o=database.odoo_version_group_id
                )
            )
        else:
            docker_image = "{o.docker_image}:{}".format(
                database.odoo_version_id.name, o=database.odoo_version_group_id
            )
        ...
        vals["services"]["odoo"]["image"] = docker_image
    ...
    def generate_compose_file(self, project_name, values):
        ...
        project = self.get_gitlab_project(
            self.config.get_var("scaffold.docker-template")
        )
    ...
    def get_repositories(self, database):
        ...
        for repo in database.repos:
            ...
            if repo.name.identification.name == "Aucune":
                ...
                url = "{}://{}:{}@{}".format(
                    base_url.split(":")[0], user, password, base_url.split(":")[1][2:]
                )

            vals = {"name": repository, "branch": branch, "url": url}
    ...
    def get_requirements(self, project_name):
        ...
        result = [e.name for e in parsed_requirements]
        print(result)
